<html>
<head>
<title>Special Codes</title>
<META HTTP-EQUIV="Content-Type" CONTENT="text/html; charset=ISO-8859-1">
<link href="<%=request.getContextPath()%>/pages/style.css" rel="stylesheet">
</head>

<jsp:useBean id="codes" class= "datapro.eibs.beans.ESD008003Message"  scope="session" />

<jsp:useBean id="error" class="datapro.eibs.beans.ELEERRMessage"  scope="session" />

<jsp:useBean id="userPO" class="datapro.eibs.beans.UserPos" scope="session" />

<script language="Javascript1.1" src="<%=request.getContextPath()%>/pages/e/javascripts/eIBS.jsp"> </SCRIPT>
<script language="Javascript1.1" src="<%=request.getContextPath()%>/pages/e/javascripts/optMenu.jsp"> </SCRIPT>

<SCRIPT Language="Javascript">

   <% 
   if ( userPO.getOption().equals("CLIENT_P") ) {
   %>
		builtNewMenu(client_inq_personal_opt);
  <%      
   }
   else
   {
   %>
		builtNewMenu(client_inq_corp_opt);
   <%
   }
   %>
	
</SCRIPT>

<body bgcolor="#FFFFFF">

 
 
 <% 
 if ( !error.getERRNUM().equals("0")  ) {
     error.setERRNUM("0");
     out.println("<SCRIPT Language=\"Javascript\">");
     out.println("       showErrors()");
     out.println("</SCRIPT>");
 }
  if ( !userPO.getPurpose().equals("NEW") ) {
    out.println("<SCRIPT> initMenu();  </SCRIPT>");
 }
%>

<h3 align="center">Classification Codes<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" ALT="client_inq_both_codes, ESD0080"></h3>
<hr size="4">
 <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/servlet/datapro.eibs.client.JSESD0080" >
  <INPUT TYPE=HIDDEN NAME="SCREEN" VALUE="32">
  
<p>
<table class="tableinfo">
  <tr > 
    <td> 
      <table cellspacing="0" cellpadding="2" width="100%" class="tbhead" bgcolor="#FFFFFF" bordercolor="#FFFFFF" bordercolorlight="#FFFFFF" bordercolordark="#FFFFFF"  align="center">
        <tr>
             <td nowrap width="10%" align="right"> Customer : 
               </td>
          <td nowrap width="12%" align="left">
      			<%= userPO.getHeader1()%>
          </td>
            <td nowrap width="6%" align="right">ID :  
            </td>
          <td nowrap width="14%" align="left">
      			<%= userPO.getHeader2()%>
          </td>
            <td nowrap width="8%" align="right"> Name : 
               </td>
          <td nowrap width="50%"align="left">
      			<%= userPO.getHeader3()%>
          </td>
        </tr>
      </table>
    </td>
  </tr>
</table>
</p> 
    <table class="tableinfo" >
      <tr > 
        <td >
          <table cellspacing="0" cellpadding="2" width="100%" border="0" >
            <tr id="trdark"> 
              <td width="30%"> 
                <div align="right">Officer :</div>
              </td>
              <td width="70%"> 
                <input type="text" readonly name="E03OFC" size="5" maxlength="3" value="<%= codes.getE03OFC().trim()%>">
                <input type="text" readonly name="D03OFC" size="36" maxlength="35" value="<%= codes.getD03OFC().trim()%>">
              </td>
            </tr>
            <tr  id="trclear"> 
              <td width="30%"> 
                <div align="right">Substitude Officer :</div>
              </td>
              <td width="70%"> 
                <input type="text" readonly name="E03OF2" size="5" maxlength="3" value="<%= codes.getE03OF2().trim()%>">
                <input type="text" readonly name="D03OF2" size="36" maxlength="35" value="<%= codes.getD03OF2().trim()%>">
              </td>
            </tr>
            <tr id="trdark"> 
              <td width="30%"> 
                <div align="right">Industry :</div>
              </td>
              <td width="70%"> 
                <input type="text" readonly name="E03INC" size="5" maxlength="3" value="<%= codes.getE03INC().trim()%>">
                <input type="text" readonly name="D03INC" size="36" maxlength="35" value="<%= codes.getD03INC().trim()%>">
              </td>
            </tr>
            <tr  id="trclear"> 
              <td width="3%"> 
                <div align="right">Business :</div>
              </td>
              <td width="70%"> 
                <input type="text" readonly name="E03BUC" size="5" maxlength="3" value="<%= codes.getE03BUC().trim()%>">
                <input type="text" readonly name="D03BUC" size="36" maxlength="35" value="<%= codes.getD03BUC().trim()%>">
              </td>
            </tr>
            <tr id="trdark"> 
              <td width="30%" height="42"> 
                <div align="right">Country of Residence :</div>
              </td>
              <td width="70%" height="42"> 
                <input type="text" readonly name="E03GEC" size="5" maxlength="3" value="<%= codes.getE03GEC().trim()%>">
                <input type="text" readonly name="D03GEC" size="36" maxlength="35" value="<%= codes.getD03GEC().trim()%>">
              </td>
            </tr>
            <tr  id="trclear"> 
              <td width="30%"> 
                <div align="right">User Code 1 :</div>
              </td>
              <td width="70%"> 
                <input type="text" readonly name="E03UC1" size="5" maxlength="3" value="<%= codes.getE03UC1().trim()%>">
                <input type="text" readonly name="D03UC1" size="36" maxlength="35" value="<%= codes.getD03UC1().trim()%>">
              </td>
            </tr>
            <tr id="trdark"> 
              <td width="30%"> 
                <div align="right">User Code 2 :</div>
              </td>
              <td width="70%"> 
                <input type="text" readonly name="E03UC2" size="5" maxlength="3" value="<%= codes.getE03UC2().trim()%>">
                <input type="text" readonly name="D03UC2" size="36" maxlength="35" value="<%= codes.getD03UC2().trim()%>">
              </td>
            </tr>
            <tr  id="trclear"> 
              <td width="30%"> 
                <div align="right">User Code 3 :</div>
              </td>
              <td width="70%"> 
                <input type="text" readonly name="E03UC3" size="5" maxlength="3" value="<%= codes.getE03UC3().trim()%>">
                <input type="text" readonly name="D03UC3" size="36" maxlength="35" value="<%= codes.getD03UC3().trim()%>">
              	(Regulation "O" Flag)
              </td>
            </tr>

            <tr id="trdark"> 
              <td width="30%"> 
                <div align="right">Census Tract :</div>
              </td>
              <td width="70%"> 
                <input type="text" readonly name="E03UC4" size="7" maxlength="6" value="<%= codes.getE03UC4().trim()%>">
              </td>
            </tr>
          </table>
        </td>
      </tr>
    </table>
</center></form>
</body>
</html>


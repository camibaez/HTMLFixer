...
function goAction() {
	...
	if (!document.forms[0].CC_Main[0].checked && !document.forms[0].CC_Main[1].checked && !document.forms[0].CC_Main[2].checked) {
		alert("Please, establish the debit card's default account");
	}
	else {
		...
		if (document.forms[0].CC_Main[0].checked && document.forms[0].CCP.value == "") {
			alert("The checking account field can't be blank");
		}
		...
		//}
			...
			alert("The saving account field can't be blank");
		}
		...
		//}
			...
			alert("The F.A.L. account field can't be blank");
		}
...
function add(num) {
	...
	var E04CCRSTP = document.getElementById("E04CCRSTP");
	if (i == -1) {
		alert("Please, select an account in the list to assign");
	}
	else {
		var key = acc[i].value;
    	var temp = new Array();
		...
		if (num == 1) {
			if (E04CCRAPC.value != "01" &&
				E04CCRAPC.value != "02" &&
				E04CCRAPC.value != "03") {
				alert("That account is not checking");
			}
			else {
				...
				if (ccp.value != "") {
					alert("There are assigned a checking account, try again");
				}
				else {
		...
		if (num == 2) {
			if (E04CCRAPC.value != "04") {
				alert("That account is not saving account");
			}
			else {
				...
				if (cap.value != "") {
					alert("There are assigned a saving account, try again");
				}
				else {
		...
		if (num == 3) {
			if (E04CCRTYP.value != "CFAL") {
				alert("That account is not F.A.L. account");
			}
			else {
				...
				if (cfp.value != "") {
					alert("There are assigned a F.A.L. account, try again");
				}
				else {
		...
		if (num == 4) {
			if (E04CCRAPC.value != "01" &&
				...
				alert("This account can't be secondary account");
			}
			else {
		...
		if (acc.length == 0) {
			var option = new Option("There are not unassigned accounts", "X", false);
...
function del(num) {
	...
	if (num == 1) {
		if (ccp.value == "") {
			alert("There are not a checking account assigned");
		}
		else {
	...
	if (num == 2) {
		if (cap.value == "") {
			alert("There are not a saving account assigned");
		}
		else {
	...
	if (num == 3) {
		if (cfp.value == "") {
			alert("There are not a F.A.L. account assigned");
		}
		else {
	...
	if (num == 4) {
		if (i == -1) {
			alert("Please, select a secondary account");
		}
		else {
	...
	if (accs.length == 0) {
		var option = new Option("There are not assigned accounts", "X", false);
		accs.options[accs.length] = option;
	}
}
//-->
...
<body>
<h3 align="center">Debit Card Assignment<BR>Additional Card Maintenance<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" alt="card_assign_maint_add.jsp, ECC0090"></h3>
<hr size="4">
<% 
...
%>
	...
	<TR>
      <TD> 
	      <div align="center"> <font size="3"><b> <FONT size="3"><B>Customer
		don't have related accounts</B></FONT></b></font></div>
      </TD>
     </TR>
   	</TABLE>
<%

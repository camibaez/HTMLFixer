...
function goAction(op) {
    ...
    if (op == 1)
      page = "<%=request.getContextPath()%>/servlet/datapro.eibs.client.JSERA0010?SCREEN=300";
	else
	  page = "<%=request.getContextPath()%>/servlet/datapro.eibs.client.JSERA0010?SCREEN=500&ROW=" + document.forms[0].ACTROW.value;
	
	CenterWindow(page,600,500,2);
 	}
 	else {
		delok = confirm("Are you sure you want to delete the selected collateral?"); 
	 	if ( delok) {
...
<BODY>
<h3 align="center"> Non-Deposit Collateral List<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" ALT="ga_acc_list.jsp,ERA0010"> 
</h3>
<hr size="4">
...
%>  
  ...
  <TABLE class="tableinfo">
    <TR id="trdark"> 
      ...
      <TH ALIGN=CENTER width="20%">Reference Number</TH>
      <TH ALIGN=CENTER width="5%">Type</TH>
      <TH ALIGN=CENTER width="20%">Maturity Date</TH>
      <TH ALIGN=CENTER width="5%">CCY</TH>
      <TH ALIGN=CENTER width="13%">Face Value</TH>
      <TH ALIGN=CENTER width="18%">Used Collaterals</TH>
      <TH ALIGN=CENTER width="18%">Used Amount</TH>
    </TR>
    <%

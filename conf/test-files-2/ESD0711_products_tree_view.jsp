			...
			<%
					...
					<table cellspacing="2" width="100%" class=tbenter>
						<tr> 
							...
							<td class="prodItem" onclick="switchOpt('<%= prodList.getCurrentRow() %>');">  												
						 	  <a href="<%=request.getContextPath()%>/pages/e/ESD0711_products_detail.jsp?appcode=<%=product.getE01APCACD().trim()%>&typecode=&generic=<%=product.getE01GENERI().trim()%>&title=<%=product.getE01DESCRI().trim()%>&bank=<%=product.getE01APCBNK()%>" target="detail">
						 	  <b>
			  ...
			  try {
			    ...
			    elemList = (datapro.eibs.beans.JBObjList) subProdList.getRecord();
			 	...
			 	while (elemList.getNextRow()) {
				  	...
				  	if( initialId == -1 ){
				  		initialId = prodList.getCurrentRow() ;
				  	}
			 ...
			 %>
			 			<tr>
			 				...
			 				<td class="optSubProd">  												
						 		<a href="<%=request.getContextPath()%>/pages/e/<%=pageDet%>?appcode=<%=subProduct.getE01APCACD().trim()%>&typecode=<%= subProduct.getE01APCTYP().trim()%>&generic=<%=subProduct.getE01GENERI().trim()%>&title=<%=subProduct.getE01DESCRI().trim()%>&bank=<%=subProduct.getE01APCBNK()%>" target="detail"><%= subProduct.getE01DESCRI().trim()%> (<%= subProduct.getE01APCTYP().trim()%>)</a>
							</td>
						</tr>
			 <%		
			 	}

<%@ page
	import="datapro.eibs.master.*,datapro.eibs.beans.*,java.util.Iterator"%>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML>
<HEAD>
<TITLE>Clauses List</TITLE>
...
<jsp:useBean id="jbList" class="datapro.eibs.beans.JBObjList"
	scope="session" />
...
<SCRIPT language="javascript">
	...
	function goDelete() {
		var ok = true;
		ok = confirm("Are you want to delete this clausula?");
		if (ok) {
...
<FORM name="letterOfCreditForm" method="post"
	action="<%=request.getContextPath()%>/servlet/datapro.eibs.products.JSEWP0010">
...
<TABLE id=tbhelp
	...
	<TR id="trdark">
		...
		<DIV align="right">Clause :</DIV>
		</TD>
		<TD align=CENTER width="34%">
		<DIV align="left"><INPUT type="text" name="CLSCDE" size="4"
			maxlength="3"></DIV>
		</TD>
	</TR>
	<TR id="trclear">
		<TD align=CENTER width="18%">
		<DIV align="right">Text NRW :</DIV>
		</TD>
		<TD align=CENTER width="34%">
		<DIV align="left"><INPUT type="text" name="CLSTXN" size="3"
			maxlength="2" onKeyPress="enterInteger()"></DIV>
		</TD>
	</TR>
	<TR id="trdark">
		<TD nowrap>
		...
		<TD nowrap><INPUT type="text" name="CLSDSC" size="47"
			maxlength="45"></TD>
	</TR>
	<TR id="trclear">
		<TD align=center nowrap colspan="2"><INPUT id="EIBSBTN"
			type=button name="Submit" value="Submit" onClick="javascript:goNew()"></TD>
	</TR>
</TABLE>
</DIV>

<H3 align="center">Clauses<IMG
	src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left"
	name="EIBS_GIF" ALT="lc_clauses_list.jsp, EWP0010"></H3>
<HR size="4">
<%
	if (jbList.size() > 0) {
%>
<TABLE class="tbenter" width="100%">
	<TR>
		...
		<div align="center" style="cursor: hand" onClick="ShowEnterCod()"><A><span
			style="color: darkblue">New</span></A></div>
		</TD>
		<TD width="25%" class=TDBKG>
		<DIV align="center"><A
			href="javascript:document.forms[0].submit();"><B>Maintenance</B></A></DIV>
		</TD>
		<TD width="25%" class=TDBKG>
		...
		<TD width="25%" class=TDBKG>
		<DIV align="center"><A
			href="<%=request.getContextPath()%>/pages/background.jsp"><B>Exit</B></A></DIV>
		</TD>
	</TR>
</TABLE>
<%
	...
	<TR>
		...
		<TABLE id="headTable" width="100%">
			<TR id="trdark">
				...
				<TH ALIGN=CENTER nowrap width="20%">Clause</TH>
				<TH ALIGN=CENTER nowrap width="20%">Text</TH>
			...
			<%
				...
				while (jbList.getNextRow()) {
					EWP001001Message msgPart = (EWP001001Message) jbList
							.getRecord();
					if (firstTime) {
						firstTime = false;
						chk = "checked";
			%>
			<INPUT TYPE=HIDDEN NAME="E01CLSCDE"
				value="<%=msgPart.getE01CLSCDE()%>">
			<INPUT TYPE=HIDDEN NAME="E01CLSTXN"
				VALUE="<%=msgPart.getE01CLSTXN()%>">
			<INPUT TYPE=HIDDEN NAME="E01CLSDSC"
				VALUE="<%=msgPart.getE01CLSDSC()%>">
			<%
				} else {
						chk = "";
					}
			...
			%>
			<TR>
				<TD NOWRAP><INPUT type="radio" name="ACCNUM_TEMP"
					value="<%=s1%>" onClick="SelectReg('<%=s1%>','<%=s2%>','<%=s3%>');"
					<%=chk%>>
				<TD NOWRAP ALIGN="CENTER"><A
					href="javascript:SelectReg('<%=s1%>','<%=s2%>','<%=s3%>');document.forms[0].submit();"><%=s1%></A></TD>
				<TD NOWRAP ALIGN="CENTER"><A
					href="javascript:SelectReg('<%=s1%>','<%=s2%>','<%=s3%>');document.forms[0].submit();"><%=s2%></A></TD>
				<TD NOWRAP ALIGN="LEFT"><A
					href="javascript:SelectReg('<%=s1%>','<%=s2%>','<%=s3%>');document.forms[0].submit();"><%=s3%></A></TD>
			</TR>
			<%
				}
			%>
...
<TABLE class="tbenter" width=100% height=40%>
	<TR>
		<TD>
		<div align="center">
		<p><b>There are not results for your search criteria</b></p>
		</div>
		</TD>
	</TR>
</TABLE>

...
function goAction() {
	...
	if (!document.forms[0].CC_Main[0].checked && !document.forms[0].CC_Main[1].checked) {
		alert("Please set the card's main account.");
	}
	else {
		...
		if (document.forms[0].CC_Main[0].checked && document.forms[0].CCP.value == "") {
			alert("A blank retail account cannot be assigned as card's main account.");
		}
		...
		//}
			...
			alert("A blank savings account cannot be assigned as card's main account.");
		}
...
function add(num) {
	...
	var E02CCRSTP = document.getElementById("E02CCRSTP");
	if (i == -1) {
		alert("Please select an account from the list to assign.");
	}
	else {
		var key = acc[i].value;
    	var temp = new Array();
		...
		if (num == 1) {
			if (E02CCRAPC.value != "01" &&
				E02CCRAPC.value != "02" &&
				E02CCRAPC.value != "03") {
				alert("The selected account is not a retail account.");
			}
			else {
				...
				if (ccp.value != "") {
					alert("There is a retail account assigned. Remove it first and try again.");
				}
				else {
		...
		if (num == 2) {
			if (E02CCRAPC.value != "04") {
				alert("The selected account is not a savings account.");
			}
			else {
				...
				if (cap.value != "") {
					alert("There is a savings account assigned. Remove it first and try again.");
				}
				else {
		...
		if (num == 4) {
			if (E02CCRAPC.value != "01" &&
				...
				alert("The selected account cannot be assigned as a secondary account.");
			}
			else {
		...
		if (acc.length == 0) {
			var option = new Option("No accounts to assign", "X", false);
...
function del(num) {
	...
	if (num == 1) {
		if (ccp.value == "") {
			alert("There is no retail account assigned.");
		}
		else {
	...
	if (num == 2) {
		if (cap.value == "") {
			alert("There is no savings account assigned.");
		}
		else {
	...
	if (num == 4) {
		if (i == -1) {
			alert("Please select a secondary account to remove.");
		}
		else {
	...
	if (accs.length == 0) {
		var option = new Option("No accounts assigned", "X", false);
		accs.options[accs.length] = option;
	}
}
//-->
...
<body>
<h3 align="center">Debit Cards Assignment<BR>Holder Card Assignment<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" alt="card_assign_maint_custom.jsp, ECC0090"></h3>
<hr size="4">
<% 
...
<form method="post" action="<%=request.getContextPath()%>/servlet/datapro.eibs.products.JSECC0090">
  ...
  <INPUT TYPE=HIDDEN NAME="E02CCRPLA" VALUE="<%= userPO.getHeader15()%>">
  <INPUT TYPE=HIDDEN NAME="E02CCRPER" VALUE="<%= userPO.getHeader16()%>">
			...
			<TABLE cellspacing="0" cellpadding="2" width="100%" border="0">
				<TBODY>
					<TR id="trdark">
						...
						<TD nowrap width="16%">
							<DIV align="right">Holder Name :</DIV>
						</TD>
						<td nowrap width="20%">
							<DIV align="left">
								<INPUT type="text" name="E02CCRNAM" size="35" maxlength="25" value="<% if (userPO.getHeader17() == "") out.print(userPO.getCusName()); else out.print(userPO.getHeader17()); %>" onKeyPress="enterCharacterText()">
							</DIV>
						</TD>
					</TR>
					<%--
					...
					<TR id="trdark">
						...
						<TD nowrap width="16%">
							<DIV align="right">Assignment Date :</DIV>
						</TD>
						<td nowrap width="20%">
							<DIV align="left">
...
%>
	...
	<TR>
      <TD> 
	      <div align="center"> <font size="3"><b> The customer does not have associated accounts.</b></font></div>
      </TD>
     </TR>
   	</TABLE>
<%
...
%>
  <table class="tableinfo">
    <tr>
	  ...
	  <% } %>
	  <% if (aux1 == 0) { %>
		<OPTION value="X">No accounts to assign</OPTION>
	  <% } %>
	  </SELECT>
	  </td>
    		...
    		<tr>
    		  <td nowrap> 
       			  <table width="100%" border="0" cellpadding="2" cellspacing="0" class="tbhead">
				        ...
				        %>
		      		  <tr id="trdark">
		        		  <th colspan="2">Card's Main Accounts</th>
						<TH>Main</TH>
					</tr>
		      		  <tr id="trclear">
		        		<td nowrap align="right">
		          			<div align="right">Main Retail Account :</div>
		          		</td>
		        		<td nowrap>
		      		  ...
		      		  <tr id="trdark">
		        		<td nowrap align="right">
		          			<div align="right">Main Savings Account :</div>
		          		</td>
		        		<td nowrap>
    ...
    <tr>
      ...
      <td align="center">      
		...
		<SELECT size="5" name="AccSList" id="AccSList" onClick="document.forms[0].AccSList.multiple = false" onBlur="document.forms[0].AccSList.multiple = true">
			...
			<% } %>
			<% if (aux2 == 0) { %>
				<OPTION value="X">No accounts assigned</OPTION>
			<% } %>
		</SELECT>
	  </td>
    </tr>

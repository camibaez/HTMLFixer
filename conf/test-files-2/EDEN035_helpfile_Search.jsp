...
function goSearch() {
	...
	window.location.href="<%=request.getContextPath()%>/pages/e/EDEN035_helpfile_Search.jsp?NameSearch="+nameSearch; 
}

function nextRecord(pos){
	var form = document.forms[0];
	var nameSearch = form.NameSearch.value;
	window.location.href="<%=request.getContextPath()%>/pages/e/EDEN035_helpfile_Search.jsp?FromRecord="+pos;
}
</script>
</head>
...
<%
	...
	if (session == null) {
		...
		String code = request.getParameter("code") == null?(String)session.getAttribute("code"):request.getParameter("code");
        ...
        try{              
			msgHelp = (EDEN03501Message) mc.getMessageRecord("EDEN03501");
			...
			if(nameSearch.length() > 0){
				...
				msgHelp.setE01DSTDSC(nameSearch);
			}
			msgHelp.setE01DSTCOU(code);
...
<TABLE  id="mainTable" class="tableinfo" ALIGN=CENTER style="width:'95%'">
	...
	<TR>
    	...
    	<% 
    		marker = ((EDEN03501Message)newmessage).getE01INDOPE();
			while (!marker.equals("*") && !marker.equals("+")) {
				...
				out.println("<td nowrap aling='center'>"
						+"<A HREF=\"javascript:enter('" + ((EDEN03501Message)newmessage).getE01DSTCDE().trim() +"', '" + 
						((EDEN03501Message)newmessage).getE01DSTDSC().trim() + "')\">"  + ((EDEN03501Message)newmessage).getE01DSTCDE().trim() + "</a></td>");
				out.println("<td nowrap aling='center'>"
						+"<A HREF=\"javascript:enter('" + ((EDEN03501Message)newmessage).getE01DSTCDE().trim() +"', '" +
						 ((EDEN03501Message)newmessage).getE01DSTDSC().trim() + "')\">"  + ((EDEN03501Message)newmessage).getE01DSTDSC().trim() + "</a></td>");
				out.println("</tr>");

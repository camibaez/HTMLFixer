	...
	switch (op) { 
	   ...
	   case  6: 
		...
		case  7: 
				...
				break;		
		case  8: 
			//SOLO SI LA PROPUESTA PERMITE RENOVACION			
			if (document.forms[0].REN.value =="1") {; 
				document.forms[0].SCREEN.value="300"; 
				document.forms[0].submit();
			}else{
				if (document.forms[0].LAN.value == 'S') {
					alert("Esta propuesta no permite ser renovada. Revise parametrizacion Producto-Ruta");
				} else {
					alert("This proposal does not permit to be renewed. Revise Product-Route parameters");
				}
...
function getParams(currrow,PROP,CUN,PSEC,PACT, TASK, PARAM, OPECOD,CUNAM,UID,USER, TACC,SUSP,TPROC,SWNODE,PESP,UESP,OPCION, REN,MOD,SEC,LAN) {
	document.forms[0].CURRENTROW.value = currrow;
    document.forms[0].PROP.value = PROP;
    document.forms[0].CUN.value = CUN;
    document.forms[0].PSEC.value = PSEC;
    document.forms[0].PACT.value = PACT;        
    ...
    document.forms[0].TACC.value = TACC; 
    document.forms[0].SUSP.value = SUSP;
    document.forms[0].TPROC.value = TPROC;
    document.forms[0].SWNODE.value = SWNODE;
    document.forms[0].PESP.value = PESP; 
    document.forms[0].UESP.value = UESP;  
    document.forms[0].REN.value = REN; 
    document.forms[0].OPCION.value = OPCION; 
    document.forms[0].CONTI.value = "0";
    document.forms[0].MOD.value = MOD;
    document.forms[0].SEC.value = SEC;
    document.forms[0].LAN.value = LAN; 
}
          ...
          <table class="tbenter" width=100% align=center>
			...
			<% if(userPO.getOption().equals("1")) { %>
              <td class=TDBKG width="30%"> 
                <div align="center"><a href="javascript:goAction(1)"><b>Apertura</b></a></div>
              </td>
			<%};%>
             <td class=TDBKG width="30%"> 
...
%> 
        ...
        <table id="headTable" width="100%">
		 ...
		 %>
          <tr> 
            <td NOWRAP  align=CENTER width="3%"> 
              ...
              value="<%= EDP072001Help.getCurrentRow() %>" 
              		 <%=chk%> onClick="getParams(this.value, 
              		   ...
              		   '<%= msgList.getE01PLPPSR() %>',
              		   '<%= msgList.getE01PLPPAC() %>',
              		   '<%= msgList.getE01DPWACC() %>',
              		   '<%= msgList.getE01DPWPAR() %>',
              		   '<%= msgList.getE01DPWOPC() %>',
              		   '<%= msgList.getE01CUSNA1() %>',
              		   '<%= msgList.getE01PLPUID() %>',
              		   '<%= msgList.getH01USERID() %>',
               		   '<%= msgList.getE01DPPTCC() %>',
               		   '<%= msgList.getE01YYYEJE() %>',
               		   '<%= msgList.getE01DPPPRO() %>',
               		   '<%= msgList.getE01DPWTAC() %>',
               		   '<%= msgList.getE01APCFL3() %>',
               		   '<%= msgList.getE01EUPLNA() %>',
               		   '<%= msgList.getH01FLGWK2() %>', 
              		   '<%= msgList.getE01DPJREN() %>',
              		   '<%= msgList.getE01XXXMOD() %>',
              		   '<%= msgList.getE01CNTSEC() %>',
              		   '<%= msgList.getE01CNTLAN() %>');">
         </td>
			...
			<td NOWRAP  align=LEFT width=\"20%\"><%= msgList.getE01PLPAM1()%><BR>
			<%= msgList.getE01PLPAM2()%></td>
         </tr>
          <%
              				chk = "";     
                }
  ...
  <%
		...
		int pos = EDP072001Help.getFirstRec() - 5;

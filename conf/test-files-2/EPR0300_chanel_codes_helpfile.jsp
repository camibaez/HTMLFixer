...
<script language="Javascript1.1" src="<%=request.getContextPath()%>/pages/s/javascripts/sortTables.jsp"> </SCRIPT>
    ...
    <table id="dataTable" > 
		<%      Socket s = null;
				...
				int rows = 0;
              	...
              	try
               	...
               	try
               	{
               		msgHelp = (EPR030201Message)mc.getMessageRecord("EPR030201");
					...
					if (newmessage.getFormatName().equals("EPR030201")) 
                  	{
                  		...
                  		{
                  	   		//newmessage = mc.receiveMessage();                  	  
                            msgHelp =  (EPR030201Message)newmessage;
		...
		%>
           	<tr>
				...
				<td nowrap><A HREF="javascript:a('<%=msgHelp.getE01PRCCNL().trim()%>', '<%=msgHelp.getE01PRCDSC().trim()%>','')"> <%=msgHelp.getE01PRCDSC().trim()%></a></td>       		           		
	        </tr>               
    	<%           	
    	       				rows++;
    	       				newmessage = mc.receiveMessage();

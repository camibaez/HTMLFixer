...
<% 
    ...
    if (typ.equals("1")) { title ="Taxes"; }
    else if (typ.equals("2")) { title ="Commissions";}
    else if (typ.equals("3")) { title ="Deductions";}
    else if (typ.equals("4")) { title ="Sales Tax";} 
    else if (typ.equals("5")) { title ="Others Charges";}  
 %>
...
<SCRIPT LANGUAGE="JavaScript">
 ...
 function updateTotal() { 
  ...
  if ( coll.length > 1 ) {
   for (var i=0; i< coll.length;i++) {      
     try{
      	tmpVal= parseFloat(replaceAll(coll[i].value,","));
      	if (isNaN(tmpVal)) {
      	  tmpVal= 0.00;
      	  coll[i].value = "0.00";
      	 }
     ...
     try{
      	totalVal= parseFloat(replaceAll(coll.value,","));
...
<H3 align="center">Payment of <%= title %> <img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" ALT="ln_renewal_chg.jsp, EDL0910"> 
</H3>
<hr size="4">
<form method="post" action="<%=request.getContextPath()%>/servlet/datapro.eibs.products.JSEDL0910">
  ...
  <p>
  <div align="center"> 
    <input id="EIBSBTN" type=button name="Submit" OnClick="SetInfo()" value="Submit">
  </div>
</p>
  <% } else { %>
  <SCRIPT Language="Javascript">

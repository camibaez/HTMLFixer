...
function goSearch() {
  window.location.href="<%=request.getContextPath()%>/pages/e/EWD0002_helpfile_CNOFC_DescF.jsp?" + document.forms[0].filters.value + "codeflag=" + document.forms[0].codFlag.value + "&FromRecord=0&SelNew=" + document.forms[0].SelNew.value + "&SelOld=" + document.forms[0].SelOld.value; 
}
//-->
</script>
</HEAD>
...
<%
                ...
                try {
               	...
               	try
               	{
               		msgHelp = (EWD0002DSMessage)mc.getMessageRecord("EWD0002DS");
               	 	...
               	 	//Agrega los filtros.  Se asume que el filtro
               	 	// es uno de los campos que vienen en los parametros de \
               	 	// de la forma.
               	 	
               	 	String[] filters= request.getParameterValues("filter");
               	 	if(null != filters){
	               	 	for (int i=0; i < filters.length ; i++){
	               	 		String paramValue= request.getParameter(filters[i]);
	               	 	  	if(null == paramValue || !paramValue.trim().equals("undefined")){
	               	 	  		System.out.println("filter["+i+"]= "+paramValue);
		               	 	 	System.out.println("filter["+i+"]= "+paramValue);
		               	 	 	msgHelp.getField(filters[i]);
		               	 	 	System.out.println(filters[i]+ ":"+paramValue);
		               	 	 	filterString=filterString+filters[i]+"="+paramValue+"&";
		               	 	}
	               	 	}
               	 	}
	  ...
	  <SCRIPT language="JavaScript">
			...
			document.forms[0].filters.value="<%= filterString %>";
	 ...
	 <TABLE  class="tbenter" WIDTH="88%" ALIGN=CENTER>
      ...
      <TD WIDTH="50%" ALIGN=LEFT height="25"> <%
        if ( posIni > 30 ) {
      			int pos = posIni - 31;
      			   out.print("<A HREF=\""+request.getContextPath()+"/pages/e/EWD0002_helpfile_CNOFC_DescF.jsp?"+filterString+"codeflag=" + codeflag + "&FromRecord=" + pos + "&SelNew=" + selNew + "&SelOld=" + selOld + "\" > <img src=\""+request.getContextPath()+"/images/e/previous_records.gif\" border=0></A>");
        } %>
      </TD>
 	  <TD WIDTH="50%" ALIGN=RIGHT height="25"> <%       
        if (marker.equals("+")) {
      			int pos = posEnd;
      			out.print("<A HREF=\""+request.getContextPath()+"/pages/e/EWD0002_helpfile_CNOFC_DescF.jsp?"+filterString+"codeflag=" + codeflag + "&FromRecord=" + pos + "&SelNew=" + selNew + "&SelOld=" + selOld + "\" ><img src=\""+request.getContextPath()+"/images/e/next_records.gif\" border=0></A>");

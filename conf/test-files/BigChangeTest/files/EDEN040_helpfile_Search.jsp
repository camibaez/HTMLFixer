...
function goSearch() {
	...
	window.location.href="<%=request.getContextPath()%>/pages/e/EDEN040_helpfile_Search.jsp?NameSearch="+nameSearch; 
}

function nextRecord(pos){
	var form = document.forms[0];
	var nameSearch = form.NameSearch.value;
	window.location.href="<%=request.getContextPath()%>/pages/e/EDEN040_helpfile_Search.jsp?FromRecord="+pos;
}
</script>
</head>
...
<%
	...
	if (session == null) {
        ...
        try{              
			msgHelp = (EDEN04001Message) mc.getMessageRecord("EDEN04001");
			...
			if(nameSearch.length() > 0){
				...
				msgHelp.setE01DENDSC(nameSearch);
			}
...
<TABLE  id="mainTable" class="tableinfo" ALIGN=CENTER style="width:'95%'">
	...
	<TR>
    	...
    	<% 
    		marker = ((EDEN04001Message)newmessage).getE01INDOPE();
			while (!marker.equals("*") && !marker.equals("+")) {
				...
				out.println("<td nowrap aling='center'>"
						+"<A HREF=\"javascript:enter('" + ((EDEN04001Message)newmessage).getE01DENCDE().trim() +"', '" + 
						((EDEN04001Message)newmessage).getE01DENDSC().trim() + "')\">"  + ((EDEN04001Message)newmessage).getE01DENCDE().trim() + "</a></td>");
				out.println("<td nowrap aling='center'>"
						+"<A HREF=\"javascript:enter('" + ((EDEN04001Message)newmessage).getE01DENCDE().trim() +"', '" +
						 ((EDEN04001Message)newmessage).getE01DENDSC().trim() + "')\">"  + ((EDEN04001Message)newmessage).getE01DENDSC().trim() + "</a></td>");
				out.println("</tr>");
